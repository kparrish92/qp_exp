...
Figures 4 is a graphical representation of the crossover boundaries of stops in the pooled data of both Spanish L1 groups.
Figure 5 shows the effect sizes derived from each of the three tests of equivalence. 
As can be seen in figure 5, the smallest effect size is the difference between the L1 and the L3, which is rather close to zero. 
...
all_tost_spanl1_stops %>% 
  ...
  geom_linerange(aes(xmin = LL_CI_TOST, xmax = UL_CI_TOST)) +
  geom_vline(xintercept = 0, linetype = "dashed") + 
  ggtitle("Figure 5: Spanish L1 stops TOST")
...
Just as in stops, Figure 6 is a graphical representation of the crossover boundaries of vowels in the pooled data of both Spanish L1 groups.
Figure 7 shows the effect sizes derived from each of the three tests of equivalence. 
Again, similarly to stops, in figure 7, the smallest effect size is the difference between the L1 and the L3, which is rather close to zero. 
...
all_data %>% 
  ...
  geom_smooth(method = "glm", method.args = list(family = "binomial")) +
  facet_grid(. ~ l1) + ylab("Proportion /u/") + xlab("Continuum Step") + 
  ggtitle("Figure 6: Spanish L1 group pooled vowels") 

all_tost_spanl1_vowels %>% 
  ggplot(aes(x = diff, y = combination)) + geom_point() + 
  geom_linerange(aes(xmin = LL_CI_TOST, xmax = UL_CI_TOST)) + 
  geom_vline(xintercept = 0, linetype = "dashed") +
  ggtitle("Figure 7: Spanish L1 Vowels TOST")
...
Just as in the previously reported tests of equivalence, figures 8 shows the graphical representation of the crossover boundaries of stops in the pooled data of both English L1 groups.
Figure 9 shows the relative effect sizes of each language combination. 
...
all_data %>% 
  ...
  geom_smooth(method = "glm", method.args = list(family = "binomial")) +
  facet_grid(. ~ l1) + ylab("Proportion /p/") + xlab("VOT") + 
  ggtitle("Figure 8: English L1 group pooled stops") 

all_tost_engl1_stops %>% 
  ggplot(aes(x = diff, y = combination)) + geom_point() + 
  geom_linerange(aes(xmin = LL_CI_TOST, xmax = UL_CI_TOST)) +
  geom_vline(xintercept = 0, linetype = "dashed") + 
  ggtitle("Figure 9: English L1 stops")
