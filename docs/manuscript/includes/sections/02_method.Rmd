...
`r m_all_3_hu %>% pull(Spanish)`) or French (in the ES group, n = `r m_all_3_fr %>% pull(English)` and in the SE group, n = `r m_all_3_fr %>% pull(Spanish)`). <!-- Throw "randomly" in there somewhere --> 
...
Additionally, participants who failed attention checks, chose the same answer for every trial, or generally displayed low effort on submissions were removed. <!-- Youll need to operationalize "low effort". --> 
The data of `r m_english_removed_stops %>% pull(n)` participants were removed for English stops, `r m_spanish_removed_stops %>% pull(n)` participants' data for Spanish stops were removed, and `r m_french_removed_stops %>% pull(n)` participants in French stops, and `r m_hungarian_removed_stops %>% pull(n)` in Hungarian stops had their data removed. <!-- The inconsistent structure of this sentence makes it read a bit odd. --> 
...
The lexTALE is a lexical decision task in which the participants are presented with either a word or a non-word, and they must decide whether the word presented is a real word or a non-word. <!-- I guess there is nothing wrong with this sentence but the repetition is a bit undesirable --> 
Both the Spanish and English versions are scored in the same manner, and allow for comparability of proficiency across groups. Both versions contain 60 words total, 40 of which are real words and 20 of which are non-words. A formula then calculates a score based on the percentage of real words correct plus the percentage of non-words correct divided by 2. Table 1 is a table of descriptive statistics which shows the mean lexTALE score per L1 group. As the table shows, the Spanish L1 group was more proficient in their L2 on average than the English L1 group. <!-- Give me the numbers in prose too --> 
Figure 1 shows a graphical relationship of the distribution of proficiency scores per L1 group. <!-- Did you do any test on the scores? You will be asked about this for sure. I ideally you want them to have similar proficiency. You need a test of equivalence (TOST) --> 
...
Participants received written instructions prior to taking part in each phase of the experiment. The instructions for the "English" task were in English, the instructions for the "Spanish" task were in Spanish, and the instructions for the L3 tasks were bilingual (both Spanish and English). <!-- explain this more. --> 
As in previous studies, the instructions contained no instances of the experimental stimuli, in this case "pafri", "bafri, "ifri, or "ufri", or any of their language specific forms. 

## Stimuli 

The 2afc tested the perception of the same two continua (a /p/-/b/ VOT continuum and a /i/-/u/ F2 continuum) in three different language modes: English, Spanish and the L3 (either Hungarian or French). The VOT continuum has been used in previous studies [@lozano-arguelles_conceptually_2020; @gonzales_how_2019] and were created from a recording by a female Spanish-English bilingual. The original VOT continuum contained the word "pafri" and "bafri". The recording was resynthesized and ranged in VOT from -35ms to 35ms, excluding a 0 VOT step. In the present study, three additional steps were added on the positive end of the scale in order to increase the chances of /p/ responses at the positive extreme of the continuum, such that steps of 40ms, 45ms, and 50ms were included and made a total of 17 total steps. <!-- Explain how you did this --> 

The stimuli come from a female native bilingual (Spanish-English) and have been used in previous studies [@casillas_perceptual_2018; @gonzales_bafri_2013; @lozano-arguelles_conceptually_2020], range from *paf*-*baf*. <!-- this seems like an incomplete sentence (and you said most of it in the previous paragraph) -->
Just as in @lozano-arguelles_conceptually_2020, the present study will conceptually cue participants, and will not utilize the language specific rhotics used in the second syllable @gonzales_bafri_2013 and @casillas_perceptual_2018. <!-- past tense --> 

The vowel continua were created using productions of the pseudowords "ifri" and "ufri" by a male American English speaker. These natural productions were then resynthesized using a PRAAT script which manipulated the second formant while holding the other steps in the continuum constant. <!-- I think you mean holding the other formants constant --> 
...
Each step of the continuum was played 5 times total, leading to a total of 85 trials in the VOT continuum per participant and 55 total trials in the stop continuum. <!-- vowel continuum --> 
Each trial was drawn randomly from the continuum in a single block, so no participant heard the same stimulus order. 
Due to language specific differences in vowel space, the non-words on screen were distinct orthographically in each language, with the exception of French and Spanish. <!-- I think you are referring to language-specific orthographic differences --> 
In Spanish and French, the participants saw *ifri* and *ufri* on the screen, in English they say *eefri* and *oofri*, and in Hungarian, they say *ifrelo* and *ufrelo*. <!-- You might need to add another sentence here to explain why this was necessary. --> 
 

## Procedure

All participants completed a total of five tasks, the Language History Questionnaire, the lexTALE proficiency test in their L2, and three two alternative forced choice tasks. <!-- You already gave the acronym way back in the intro --> 
First, the participants completed the Language History Questionnaire online [@li_language_2020], followed by the lexTALE proficiency test. 
