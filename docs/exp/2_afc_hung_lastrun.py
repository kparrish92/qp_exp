...
expName = '2_afc_hung'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\gallo\\Desktop\\qp_exp\\exp\\2_afc_hung_lastrun.py',
...
Fifrelő_p = visual.TextStim(win=win, name='Fifrelő_p',
    text='Fifrelő',
    font='Arial',
...
Fifrelő = visual.TextStim(win=win, name='Fifrelő',
    text='Fifrelő',
    font='Arial',
    pos=[-0.4, -0.2], height=0.1, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
    depth=-3.0);
Fufrelő = visual.TextStim(win=win, name='Fufrelő',
    text='Fufrelő\n',
    font='Arial',
    pos=[0.4, -0.25], height=0.1, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
...
for thisTrial in trials:
    ...
    _resp_allKeys = []
    # keep track of which components have finished
    practiceComponents = [cross_tr, sound_1, resp, Fifrelő_p, fufri_practice]
    ...
    while continueRoutine:
        ...
        # *Fifrelő_p* updates
        if Fifrelő_p.status == NOT_STARTED and tThisFlip >= .5-frameTolerance:
            # keep track of start time/frame for later
            Fifrelő_p.frameNStart = frameN  # exact frame index
            Fifrelő_p.tStart = t  # local t and not account for scr refresh
            Fifrelő_p.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(Fifrelő_p, 'tStartRefresh')  # time at next scr refresh
            Fifrelő_p.setAutoDraw(True)
    ...
    trials.addData('Fifrelő_p.started', Fifrelő_p.tStartRefresh)
    trials.addData('Fifrelő_p.stopped', Fifrelő_p.tStopRefresh)
...
for thisTrial_2 in trials_2:
    ...
    _resp_2_allKeys = []
    # keep track of which components have finished
    i_uComponents = [cross_tr_2, sound_2, resp_2, Fifrelő, Fufrelő]
    ...
    while continueRoutine:
        ...
        # *Fifrelő* updates
        if Fifrelő.status == NOT_STARTED and tThisFlip >= .5-frameTolerance:
            # keep track of start time/frame for later
            Fifrelő.frameNStart = frameN  # exact frame index
            Fifrelő.tStart = t  # local t and not account for scr refresh
            Fifrelő.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(Fifrelő, 'tStartRefresh')  # time at next scr refresh
            Fifrelő.setAutoDraw(True)
        
        # *Fufrelő* updates
        if Fufrelő.status == NOT_STARTED and tThisFlip >= .5-frameTolerance:
            # keep track of start time/frame for later
            Fufrelő.frameNStart = frameN  # exact frame index
            Fufrelő.tStart = t  # local t and not account for scr refresh
            Fufrelő.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(Fufrelő, 'tStartRefresh')  # time at next scr refresh
            Fufrelő.setAutoDraw(True)
    ...
    trials_2.addData('Fifrelő.started', Fifrelő.tStartRefresh)
    trials_2.addData('Fifrelő.stopped', Fifrelő.tStopRefresh)
    trials_2.addData('Fufrelő.started', Fufrelő.tStartRefresh)
    trials_2.addData('Fufrelő.stopped', Fufrelő.tStopRefresh)
