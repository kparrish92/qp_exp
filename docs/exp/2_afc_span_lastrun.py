...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\gallo\\Desktop\\qp_exp\\exp\\2_afc_span_lastrun.py',
...
instrText = visual.TextStim(win=win, name='instrText',
    text='Instrucciones de práctica - use la tecla de flecha derecha o izquierda para contestar. Usted va a escuchar al principio de una palabra rara en español. Usted tiene que decider el principio de qué palabra acaba de esuchar.  ',
    font='Arial',
...
fifri_p = visual.TextStim(win=win, name='fifri_p',
    text='fifri',
    font='Arial',
    ...
    languageStyle='LTR',
    depth=-3.0);
fufri_practice = visual.TextStim(win=win, name='fufri_practice',
    text='fufri\n',
    font='Arial',
...
gotIT = visual.TextStim(win=win, name='gotIT',
    text='¿Entendido? Ahora empieza el experimento de verdad. \n\nAhora, Usted va a escuchar un poco más de 50 palabras. Las instrucciones son iguales. \n\nPresiona la barra espaciadora para emepzar. \n\n\n',
    font='Arial',
...
fifri_p_2 = visual.TextStim(win=win, name='fifri_p_2',
    text='fifri',
    font='Arial',
    pos=[-0.4, -0.2], height=0.1, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
    depth=-3.0);
fufri_practice_2 = visual.TextStim(win=win, name='fufri_practice_2',
    text='fufri\n',
    font='Arial',
    pos=[0.4, -0.25], height=0.1, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
...
instrText_2 = visual.TextStim(win=win, name='instrText_2',
    text='Bien! Ahora tenemos el mismo experimento con nuevas palabras. Hay 75 palabras esta vez. Presiona la barra espaciadora para empezar. ',
    font='Arial',
    pos=[0, 0], height=0.1, wrapWidth=None, ori=0, 
    color=[1, 1, 1], colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
...
for thisTrial in trials:
    ...
    _resp_allKeys = []
    # keep track of which components have finished
    practiceComponents = [cross_tr, sound_1, resp, fifri_p, fufri_practice]
    ...
    while continueRoutine:
        ...
        # *fifri_p* updates
        if fifri_p.status == NOT_STARTED and tThisFlip >= .5-frameTolerance:
            # keep track of start time/frame for later
            fifri_p.frameNStart = frameN  # exact frame index
            fifri_p.tStart = t  # local t and not account for scr refresh
            fifri_p.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(fifri_p, 'tStartRefresh')  # time at next scr refresh
            fifri_p.setAutoDraw(True)
        
        # *fufri_practice* updates
        if fufri_practice.status == NOT_STARTED and tThisFlip >= .5-frameTolerance:
            # keep track of start time/frame for later
            fufri_practice.frameNStart = frameN  # exact frame index
            fufri_practice.tStart = t  # local t and not account for scr refresh
            fufri_practice.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(fufri_practice, 'tStartRefresh')  # time at next scr refresh
            fufri_practice.setAutoDraw(True)
    ...
    trials.addData('fifri_p.started', fifri_p.tStartRefresh)
    trials.addData('fifri_p.stopped', fifri_p.tStopRefresh)
    trials.addData('fufri_practice.started', fufri_practice.tStartRefresh)
    trials.addData('fufri_practice.stopped', fufri_practice.tStopRefresh)
...
for thisTrial_2 in trials_2:
    ...
    _resp_2_allKeys = []
    # keep track of which components have finished
    i_uComponents = [cross_tr_2, sound_2, resp_2, fifri_p_2, fufri_practice_2]
    ...
    while continueRoutine:
        ...
        # *fifri_p_2* updates
        if fifri_p_2.status == NOT_STARTED and tThisFlip >= .5-frameTolerance:
            # keep track of start time/frame for later
            fifri_p_2.frameNStart = frameN  # exact frame index
            fifri_p_2.tStart = t  # local t and not account for scr refresh
            fifri_p_2.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(fifri_p_2, 'tStartRefresh')  # time at next scr refresh
            fifri_p_2.setAutoDraw(True)
        
        # *fufri_practice_2* updates
        if fufri_practice_2.status == NOT_STARTED and tThisFlip >= .5-frameTolerance:
            # keep track of start time/frame for later
            fufri_practice_2.frameNStart = frameN  # exact frame index
            fufri_practice_2.tStart = t  # local t and not account for scr refresh
            fufri_practice_2.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(fufri_practice_2, 'tStartRefresh')  # time at next scr refresh
            fufri_practice_2.setAutoDraw(True)
    ...
    trials_2.addData('fifri_p_2.started', fifri_p_2.tStartRefresh)
    trials_2.addData('fifri_p_2.stopped', fifri_p_2.tStopRefresh)
    trials_2.addData('fufri_practice_2.started', fufri_practice_2.tStartRefresh)
    trials_2.addData('fufri_practice_2.stopped', fufri_practice_2.tStopRefresh)
