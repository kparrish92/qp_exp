...
temp_vowel_df_eng <- read_csv(here("data","5_eng.csv")) %>% 
...
temp_stops_df_eng <- read_csv(here("data","5_eng.csv")) %>% 
...
temp_vowel_df_span <- read_csv(here("data","5_spa.csv")) %>% 
  filter(!is.na(resp_2.keys) | !is.na(resp_3.keys)) %>% 
  mutate(exp = if_else(is.na(resp_2.keys), "stops", "vowels")) %>% 
  separate(stimuli, into = c("trash1", "step"), sep = "_") %>% 
  separate(step, into = c("continuum", "trash2"), sep = 2) %>% 
...
temp_stops_df_span <- read_csv(here("data","5_spa.csv")) %>% 
  filter(!is.na(resp_3.keys)) %>% 
  mutate(exp = "stops") %>% 
  separate(stimuli, into = c("trash1", "step"), sep = "/") %>% 
  separate(step, into = c("trash", "step2"), sep = 2) %>% 
...
temp_vowel_df_fren <- read_csv(here("data","5_fre.csv")) %>% 
  filter(!is.na(resp_2.keys) | !is.na(resp_3.keys)) %>% 
  mutate(exp = if_else(is.na(resp_2.keys), "stops", "vowels")) %>% 
  separate(stimuli, into = c("trash1", "step"), sep = "_") %>% 
  separate(step, into = c("continuum", "trash2"), sep = 2) %>% 
...
temp_stops_df_fren <- read_csv(here("data","5_fre.csv")) %>% 
  filter(!is.na(resp_3.keys)) %>% 
  mutate(exp = "stops") %>% 
  separate(stimuli, into = c("trash1", "step"), sep = "/") %>% 
  separate(step, into = c("trash", "step2"), sep = 2) %>% 
...
stops = ggplot(temp_stops_df_eng,aes (x = step_cont, y = resp_stop)) + geom_smooth(method = "glm", method.args = list(family = "binomial")) + geom_smooth(data = temp_stops_df_span, method = "glm", method.args = list(family = "binomial"), color = "red") + geom_smooth(data = temp_stops_df_fren, method = "glm", method.args = list(family = "binomial"), color = "yellow") + ggtitle("vowels")


grid.arrange(vowels, stops, ncol =2, bottom="Eng=blue, Span=red, Fren=yellow")
